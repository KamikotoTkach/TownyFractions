  ...
  public void onEnable() {
    ...
    new Command("prison")
       .arguments(
          ...
          ).canExecute(IS_MINISTRO_DELLA_POLIZIA.or(ServerOperator::isOp)).help("Создать тюрьму"),
   
          new ArgumentSet(
             new DeletePrison(),
             new ExactStringArg("delete"),
             new PrisonsArg().optional()
          ).canExecute(IS_MINISTRO_DELLA_POLIZIA.or(ServerOperator::isOp)).help("Удалить тюрьму"),
   
          new ArgumentSet(
             new PrisonArrest(),
             new ExactStringArg("arrest"),
             new PrisonsArg().optional()
          ).canExecute(IS_POLICEMAN.or(IS_MILITARY)).help("Арестовать игрока"),
   
          new ArgumentSet(
             new PrisonDemobilize(),
             new ExactStringArg("free"),
             new PrisonersArg()
          ).canExecute(IS_MINISTRO_DELLA_POLIZIA.or(ServerOperator::isOp)).help("Освободить игрока")
    ...
    new Command("raid")
       .arguments(new ArgumentSet(new RaidCommand(), new AllTerritories()).canExecute(CAN_RAID).help("Начать рейд")
       ).register(this);
